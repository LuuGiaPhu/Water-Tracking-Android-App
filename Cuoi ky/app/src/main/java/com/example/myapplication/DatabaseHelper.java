...
//                db.close();
        ...
        public List<Integer> getTodayWaterIntake() {
            ...
            Cursor cursor = null;
            try {
                db = getReadableDatabaseInstance(); // Use synchronized method
                ...
                Log.e("DatabaseHelper", "Database is locked. Retrying...", e);
                try {
                ...
                if (db != null && db.isOpen()) {
                    // Do not close the database here if shared across the app
                }
            }
...
//            db.close();
        ...
        public void deleteWaterIntakeRecord(int id) {
            SQLiteDatabase db = this.getWritableDatabase();
            try {
                // Log the id being used for deletion
                Log.d("DatabaseHelper", "Attempting to delete record with id: " + id);

                // Execute the delete query
                int rowsDeleted = db.delete(TABLE_WATER_INTAKE, COLUMN_INTAKE_ID + " = ?", new String[]{String.valueOf(id)});

                // Log the number of rows deleted
                Log.d("DatabaseHelper", "Rows deleted: " + rowsDeleted);

                if (rowsDeleted == 0) {
                    Log.w("DatabaseHelper", "No record found with the given id: " + id);
                }
            } catch (Exception e) {
                Log.e("DatabaseHelper", "Error deleting record with id: " + id, e);
            }
        }
